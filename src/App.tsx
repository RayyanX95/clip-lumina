...
function App() {
  ...
  const sortedHistory = [...history].sort((a, b) => {
      if (a.pinned === b.pinned) return 0; // Keep chronological order within groups
      return a.pinned ? -1 : 1;
  });
          ...
          sortedHistory.map((item) => (
            ...
            >
              <div className="flex items-start gap-3">
                <div className={`mt-1 w-1 h-1 rounded-full transition-colors ${
                    item.pinned ? "bg-brand-accent shadow-[0_0_8px_var(--color-accent)] w-1.5 h-1.5" : "bg-brand-primary/50 group-hover:bg-brand-primary"
                }`} />
                ...
                <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute right-2 top-2 flex flex-col gap-1 bg-brand-bg-deep/80 backdrop-blur-sm rounded-lg border border-white/5 p-0.5 shadow-xl translate-x-1 group-hover:translate-x-0">
                   <button 
                     ...
                     className={`p-1.5 rounded-md transition-colors ${
                         item.pinned ? "text-brand-accent bg-brand-accent/10 hover:bg-brand-accent/20" : "text-brand-text-dim hover:text-brand-accent hover:bg-brand-accent/5"
                     }`}
